...
export default function APIKeyForm(){
    ...
    const {tokens, setPopup, current_project, userInfo} = useContext(UserContext)

    async function addCategory(category){
        try {
            const {data} = await authFetchData(tokens.access_token).post(`/auth/api-key/${userInfo.id}`, category)
    ...
    const formik = useFormik({
        ...
        validationSchema: Yup.object({
            name: Yup.string().max(50, "Name should not exceed 50 characters").required("Name is required."),
        })
    })
    return (
        <main className="mt-8 w-full lg:w-1/2">
            <form onSubmit={formik.handleSubmit} className="mx-8">
                <div className="text-left pt-2">
                    <label className="block text-[#fff] text-md mb-1 font-bold" htmlFor="name">{formik.touched.name &&formik.errors.name ?formik.errors.name: "Key Name"}</label>
                    <input
